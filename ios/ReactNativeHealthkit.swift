...
class ReactNativeHealthkit: RCTEventEmitter {
  ...
  func getRequestStatusForAuthorization(
    toShare: NSDictionary, read: NSDictionary, resolve: @escaping RCTPromiseResolveBlock,
  ...
  func getPreferredUnits(
    forIdentifiers: NSArray, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func authorizationStatusFor(
    typeIdentifier: String, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func saveQuantitySample(
    typeIdentifier: String, unitString: String, value: Double, start: Date, end: Date,
    metadata: [String: Any], resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func deleteQuantitySample(
    typeIdentifier: String, uuid: String, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func deleteSamples(
    typeIdentifier: String, start: Date, end: Date, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
    ...
    let samplePredicate = HKQuery.predicateForSamples(
      withStart: start, end: end, options: HKQueryOptions.strictStartDate)
  ...
  func saveCorrelationSample(
    typeIdentifier: String, samples: [[String: Any]], start: Date, end: Date,
    metadata: [String: Any], resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      ...
      if sample.keys.contains("quantityType") {
        ...
        if let type = HKSampleType.quantityType(forIdentifier: typeId) {
          ...
          let quantitySample = HKQuantitySample.init(
            type: type, quantity: quantity, start: start, end: end, metadata: metadata)
        ...
        if let type = HKSampleType.categoryType(forIdentifier: typeId) {
          ...
          let categorySample = HKCategorySample.init(
            type: type, value: value, start: start, end: end, metadata: metadata)
    ...
    let correlation = HKCorrelation.init(
      type: type, start: start, end: end, objects: initializedSamples, metadata: metadata)
  ...
  func saveWorkoutSample(
    typeIdentifier: UInt, quantities: [[String: Any]], start: Date, end: Date,
    metadata: [String: Any], resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
    ...
    if totalSwimmingStrokeCount != nil {
      workout = HKWorkout.init(
        activityType: type, start: start, end: end, workoutEvents: nil,
        totalEnergyBurned: totalEnergyBurned, totalDistance: totalDistance,
        totalSwimmingStrokeCount: totalSwimmingStrokeCount, device: nil, metadata: metadata)
    } else {
      if #available(iOS 11, *) {
        if totalFlightsClimbed != nil {
          workout = HKWorkout.init(
            activityType: type, start: start, end: end, workoutEvents: nil,
            totalEnergyBurned: totalEnergyBurned, totalDistance: totalDistance,
            totalFlightsClimbed: totalFlightsClimbed, device: nil, metadata: metadata)
        }
      }
    }
    ...
    if workout == nil {
      workout = HKWorkout.init(
        activityType: type, start: start, end: end, workoutEvents: nil,
        totalEnergyBurned: totalEnergyBurned, totalDistance: totalDistance, metadata: metadata)
    }
  ...
  func saveCategorySample(
    typeIdentifier: String, value: Double, start: Date, end: Date, metadata: NSDictionary,
    resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
    ...
    let sample = HKCategorySample.init(
      type: type, value: Int(value), start: start, end: end, metadata: metadata as? [String: Any])
  ...
  func enableBackgroundDelivery(
    typeIdentifier: String, updateFrequency: Int, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func subscribeToObserverQuery(
    ...
    let predicate = HKQuery.predicateForSamples(
      withStart: Date.init(), end: nil, options: HKQueryOptions.strictStartDate)
  ...
  func unsubscribeQuery(
    queryId: String, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func queryStatisticsForQuantity(
    typeIdentifier: String, unitString: String, from: Date, to: Date, options: NSArray,
    resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
    ...
    let predicate = HKQuery.predicateForSamples(
      withStart: from, end: to, options: HKQueryOptions.strictEndDate)
    ...
    let query = HKStatisticsQuery.init(
      quantityType: quantityType, quantitySamplePredicate: predicate, options: opts
      ...
      if let gottenStats = stats {
        ...
        if let sumQuantity = gottenStats.sumQuantity() {
          dic.updateValue(
            serializeQuantity(unit: unit, quantity: sumQuantity), forKey: "sumQuantity")
        }
        if #available(iOS 12, *) {
        ...
        if #available(iOS 13, *) {
          ...
          if let duration = gottenStats.duration() {
            dic.updateValue(
              serializeQuantity(unit: durationUnit, quantity: duration), forKey: "duration")
          }
        }
  ...
  func queryStatisticsCollection(
    ...
    // Convert typeIdentifier into HKQuantityType
    guard
  ...
  func queryWorkoutSamples(
    energyUnitString: String, distanceUnitString: String, from: Date, to: Date, limit: Int,
    ...
    let q = HKSampleQuery(
      sampleType: .workoutType(), predicate: predicate, limit: limit,
        ...
        for s in samples {
          if let workout = s as? HKWorkout {
            ...
            let dict: NSMutableDictionary = [
              ...
              "totalDistance": serializeQuantity(
                unit: distanceUnit, quantity: workout.totalDistance) as Any,
              "totalEnergyBurned": serializeQuantity(
                unit: energyUnit, quantity: workout.totalEnergyBurned) as Any,
              ...
              "sourceRevision": serializeSourceRevision(_sourceRevision: workout.sourceRevision)
                as Any,
            ]
  ...
  func queryCorrelationSamples(
    typeIdentifier: String, from: Date, to: Date, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
                ...
                if let quantitySample = o as? HKQuantitySample {
                  objects.add(
                    serializeQuantitySample(
                      sample: quantitySample, unit: map[quantitySample.quantityType]!))
                }
  ...
  func queryCategorySamples(
    typeIdentifier: String, from: Date, to: Date, limit: Int, ascending: Bool,
    resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
      ...
      guard let err = error else {
        ...
        return resolve([
          "samples": samples.map({ sample in
            let serialized = serializeQuantitySample(
              sample: sample as! HKQuantitySample, unit: HKUnit.init(from: unitString))
  ...
  func queryCategorySamplesWithAnchor(
    typeIdentifier: String, from: Date, to: Date, limit: Int, anchor: String,
    resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock
  ) {
    guard let store = _store else {
      return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
  ...
  func querySources(
    ...
    let query = HKSourceQuery(sampleType: type as! HKSampleType, samplePredicate: nil) {
      (_: HKSourceQuery, source: Set<HKSource>?, error: Error?) in
      guard let err = error else {
  ...
  @available(iOS 13.0.0, *)
  func getSerializedWorkoutLocations(store: HKHealthStore, workoutUUID: UUID) async -> [[String:
    Any]]?
  {
    ...
    for route in _routes {
      ...
      let routeLocations = routeCLLocations.enumerated().map { (i, loc) in
        serializeLocation(location: loc, previousLocation: i == 0 ? nil : routeCLLocations[i - 1])
      }
  ...
  typealias HKAnchoredObjectQueryResult = (
    samples: [HKSample], deletedSamples: [HKDeletedObject]?, newAnchor: HKQueryAnchor?
  )
      ...
      let query = HKAnchoredObjectQuery(
        ...
        continuation.resume(
          returning: HKAnchoredObjectQueryResult(
            samples: samples, deletedSamples: deletedSamples, newAnchor: newAnchor))
      }
  ...
  @available(iOS 13.0.0, *)
  func getHeartbeatSeriesHeartbeats(store: HKHealthStore, sample: HKHeartbeatSeriesSample)
  ...
  @available(iOS 13.0.0, *)
  func getSerializedHeartbeatSeriesSample(store: HKHealthStore, sample: HKHeartbeatSeriesSample)
    async throws -> [String: Any]
  {
    ...
    Task {
      do {
        ...
        let queryResult = try await _queryHeartbeatSeriesSamplesWithAnchor(
          store: store, predicate: predicate, limit: limit, anchor: actualAnchor)

        var allHeartbeatSamples: [[String: Any]] = []
        for sample in queryResult.samples as! [HKHeartbeatSeriesSample] {
          allHeartbeatSamples.append(
            try await getSerializedHeartbeatSeriesSample(store: store, sample: sample))
        }
    ...
    Task {
      do {
        ...
        let samples = try await _queryHeartbeatSeriesSamples(
          store: store, predicate: predicate, limit: limit, ascending: ascending)
